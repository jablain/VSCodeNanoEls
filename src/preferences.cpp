...
int savedMode; // mode saved in Preferences
long savedDupr; // dupr saved in Preferences
long savedMoveStep; // moveStep saved in Preferences
int savedStarts; // starts saved in Preferences
unsigned long saveTime; // micros() of the previous Prefs write
int savedMeasure; // measure value saved in Preferences
float savedConeRatio; // value of coneRatio saved in Preferences
int savedTurnPasses; // value of turnPasses saved in Preferences
bool savedAuxForward; // value of auxForward saved in Preferences
long savedSpindlePosAvg; // spindlePosAvg saved in Preferences
long savedSpindlePos; // spindlePos value saved in Preferences
int savedSpindlePosSync; // spindlePosSync saved in Preferences
long savedSpindlePosGlobal; // spindlePosGlobal saved in Preferences
bool savedShowAngle; // showAngle value saved in Preferences
bool savedShowTacho; // showTacho value saved in Preferences
...
bool preferencesUnchanged() {
  return
    (dupr == savedDupr &&
    starts == savedStarts &&
    z.pos == z.savedPos &&
    z.originPos == z.savedOriginPos &&
    z.posGlobal == z.savedPosGlobal &&
    z.motorPos == z.savedMotorPos &&
    z.leftStop == z.savedLeftStop &&
    z.rightStop == z.savedRightStop &&
    z.disabled == z.savedDisabled &&
    spindlePos == savedSpindlePos &&
    spindlePosAvg == savedSpindlePosAvg &&
    spindlePosSync == savedSpindlePosSync &&
    savedSpindlePosGlobal == spindlePosGlobal &&
    showAngle == savedShowAngle &&
    showTacho == savedShowTacho &&
    moveStep == savedMoveStep &&
    mode == savedMode &&
    measure == savedMeasure &&
    x.pos == x.savedPos &&
    x.originPos == x.savedOriginPos &&
    x.posGlobal == x.savedPosGlobal &&
    x.motorPos == x.savedMotorPos &&
    x.leftStop == x.savedLeftStop &&
    x.rightStop == x.savedRightStop &&
    x.disabled == x.savedDisabled &&
    a1.pos == a1.savedPos &&
    a1.originPos == a1.savedOriginPos &&
    a1.posGlobal == a1.savedPosGlobal &&
    a1.motorPos == a1.savedMotorPos &&
    a1.leftStop == a1.savedLeftStop &&
    a1.rightStop == a1.savedRightStop &&
    a1.disabled == a1.savedDisabled &&
    coneRatio == savedConeRatio &&
    turnPasses == savedTurnPasses &&
    savedAuxForward == auxForward);
}

bool okToWrite () {
  unsigned long now = micros(); 
  return (!stepperIsRunning(&z) && // Z axis is not moving
          ...
          ((now < saveTime) || now > (saveTime + SAVE_DELAY_US)) && // The last save was more than 5 seconds ago
          ((now < keypadTimeUs) || now > (keypadTimeUs + SAVE_DELAY_US))); // The last key was pressed more than 5 seconds ago
}

bool writePreferences() {
  // Avoid writing Preferences when possible to reduce memory wear and
  // avoid ~20ms write delay that blocks interrupts.
  if (!okToWrite() || (preferencesUnchanged()))
    return false;
