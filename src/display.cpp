...
void taskDisplay(void *param) {
  while (emergencyStop == ESTOP_NONE) {
    ...
    if (!stepperIsRunning(&z) && // Z axis is not moving
        ...
        ((now < saveTime) || now > (saveTime + SAVE_DELAY_US)) && // Has it been more than 5 seconds since the last save
        ((now < kpadTimeUs()) || now > (kpadTimeUs() + SAVE_DELAY_US))) { // was the last key pressed more than 5 seconds ago
      if (saveIfChanged()) {
        saveTime = now;
        powerOnBeep();
        powerOnBeep();
