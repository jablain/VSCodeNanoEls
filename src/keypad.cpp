...
void buttonOnOffPress(bool on) {
  ...
  if (on && isPassMode() && (missingZStops || x.leftStop == LONG_MAX || x.rightStop == LONG_MIN)) 
    warningBeep();
  else if (!isOn && on && setupIndex < getLastSetupIndex()) // Move to the next setup step.
    setupIndex++;
  else if (isOn && on && (mode == MODE_TURN || mode == MODE_FACE || mode == MODE_THREAD)) // Move to the next pass.
...
void resetNumpad() { numpadIndex = 0; }
...
long normalizePitch(long pitch) {
  ...
  if (measure == MEASURE_METRIC)
    scale = 100; // Drop the 3rd and 4th precision point if any.
  else if (measure == MEASURE_INCH)
    scale = 254; // Always drop the 4th precision point in inch representation if any.
...
void buttonLeftStopPress (Axis* a) {setLeftStop(a, a->leftStop == LONG_MAX ? a->pos : LONG_MAX); }
void buttonRightStopPress(Axis* a) {setRightStop(a, a->rightStop == LONG_MIN ? a->pos : LONG_MIN);}
...
void processKeypadEvent() {
  ...
  if (mode == MODE_GCODE && isOn) { // Not allowed to interfere other than turn off.
...
void setupKeypad() {
  ...
  keypad.matrix(7, 7);
  keypad.flush();
  delay(100); // Give time for the hardware to initialize
